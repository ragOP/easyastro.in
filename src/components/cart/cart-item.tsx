...
function CartItemInner({ item, onRemove, showRemoveButton = true }: CartItemProps) {
  ...
  return (
    <div className={isValentine ? "rounded-2xl shadow-lg border-2 border-rose-500/30 bg-gradient-to-br from-rose-950/80 to-rose-950/50 overflow-hidden" : "bg-card rounded-2xl shadow-lg border border-border overflow-hidden"}>
      <div className="px-4 py-4">
        ...
        <div className="flex items-start justify-end">
          {showRemoveButton && (
            <button
              ...
              className={isValentine ? "p-2 text-rose-300 hover:text-rose-200 transition-colors duration-200" : "p-2 text-muted-foreground hover:text-primary transition-colors duration-200"}
            >
              <X className="w-5 h-5" />
            </button>
          )}
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className={isValentine ? "text-sm text-rose-200/80" : "text-sm text-muted-foreground"}>
                {item.description}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2 mb-2 pb-4">
            {item.features.map((feature, index) => (
              <div key={index} className="flex items-center space-x-2">
                <Heart className={isValentine ? "w-3 h-3 text-rose-400 flex-shrink-0" : "w-3 h-3 text-primary flex-shrink-0"} />
                <span className={isValentine ? "text-xs text-rose-200/90" : "text-xs text-foreground/80"}>
                  {feature}
                </span>
              </div>
            ))}
        ...
        {/* Price */}
        <div className={isValentine ? "flex items-center justify-between pt-2 border-t border-rose-500/30" : "flex items-center justify-between pt-2 border-t border-border"}>
          <div className="flex items-center space-x-3">
            ...
            {discountPercentage > 0 ? (
              <div className="flex items-center space-x-2">
                ...
                <span className={isValentine ? "text-xs bg-rose-500/30 text-rose-200 px-2 py-1 rounded-full font-semibold" : "text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full font-semibold"}>
                  COUPON APPLIED
                </span>
              </div>
            ) : (
