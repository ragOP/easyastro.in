  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        ...
        const res = await fetch(`${BACKEND_URL}/api/lander5/details?txnid=${encodeURIComponent(txnid)}`);
        ...
        if (json?.success && json?.data) {
          ...
          try {
            const deleteAbdOrder = await fetch(`${BACKEND_URL}/api/lander5/delete-order-abd`, {
              method: "DELETE",
              headers: {
